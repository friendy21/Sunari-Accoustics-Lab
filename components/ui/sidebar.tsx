...
const sidebarVariants = cva("bg-zinc-900 border-r border-zinc-800 h-full flex flex-col", {
  variants: {
    variant: {
      default: "w-64",
      narrow: "w-20",
...
const SidebarFooter = React.forwardRef<HTMLDivElement, React.HTMLAttributes<HTMLDivElement>>(
  ({ className, ...props }, ref) => (
    <div ref={ref} className={cn("p-4 border-t border-zinc-800", className)} {...props} />
  ),
)
  ...
  "flex items-center gap-3 w-full px-3 py-2 text-sm font-medium rounded-md transition-colors",
  {
    variants: {
      isActive: {
        ...
        false: "text-zinc-400 hover:text-white hover:bg-zinc-800/50",
      },
    },
    defaultVariants: {
      isActive: false,
...
const SidebarRail = React.forwardRef<HTMLDivElement, React.HTMLAttributes<HTMLDivElement>>(
  ({ className, ...props }, ref) => (
    <div ref={ref} className={cn("absolute top-0 bottom-0 right-0 w-px bg-zinc-800", className)} {...props} />
  ),
)
